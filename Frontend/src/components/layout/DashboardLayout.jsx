...
import { LayoutDashboard, Heart, Settings, LogOut, Menu, X, User, ShieldCheck, Globe } from 'lucide-react';
...
const DashboardLayout = ({ children, role }) => {
  ...
  return (
    <div className="min-h-dvh min-w-0 w-full overflow-x-clip bg-[#F8FAFC] flex font-sans">
      ...
      `}>
        <div className="h-24 flex items-center px-6 sm:px-8 border-b border-slate-100 gap-2 min-w-0">
          <Link to="/" className="flex items-center gap-3 group min-w-0 flex-1">
            <div className="w-10 h-10 bg-primary-900 rounded-xl flex items-center justify-center shadow-lg shadow-primary-900/20 group-hover:scale-105 transition-transform shrink-0">
              <ShieldCheck className="w-6 h-6 text-white" />
            </div>
            <div className="flex flex-col min-w-0">
              <span className="font-serif font-bold text-lg sm:text-xl text-slate-900 tracking-tight leading-none truncate">FoodRescue</span>
              <span className="text-[9px] font-bold text-slate-400 uppercase tracking-[0.2em] sm:tracking-[0.3em] mt-1.5 truncate">Ops Terminal</span>
            </div>
          </Link>
            <button
              className="lg:hidden ml-auto p-2 text-slate-400 hover:text-slate-600 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 rounded-lg"
        ...
        <div className="p-6 border-t border-slate-100">
          <Link
            to="/profile"
            className="flex items-center gap-4 p-4 rounded-2xl bg-slate-50 border border-slate-100 mb-4 group hover:border-primary-200 transition-colors"
          >
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-x-hidden">
        <header className="min-h-20 sm:min-h-24 bg-white/80 backdrop-blur-md border-b border-slate-200/50 flex flex-wrap items-center justify-between gap-y-3 gap-x-4 px-4 sm:px-8 lg:px-12 sticky top-0 z-30 transition-all">
          <div className="flex items-center gap-4 sm:gap-6 min-w-0 flex-1 basis-[min(100%,24rem)]">
            <button
              className="lg:hidden p-2.5 bg-slate-100 rounded-xl text-slate-600 hover:bg-slate-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 transition-colors"
            ...
            <div className="min-w-0">
               <div className="flex items-center gap-2 mb-1 min-w-0">
                  <h2 className="text-lg sm:text-xl font-serif font-bold tracking-tight text-slate-900 truncate">
               ...
               <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em]">Verified session active</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 sm:gap-4 shrink-0 ml-auto">
            ...
            <NotificationCenter
              userId={user?.id}
              role={user?.role === 'NGO' ? 'NGO' : 'DONOR'}
            />
          </div>
        </header>

        <main className="flex-1 overflow-y-auto overflow-x-hidden p-4 sm:p-8 lg:p-12 pb-24 lg:pb-32 min-w-0">
